...
def main():
    ...
    tree_reg.fit(weather_train, weather_labels)

    # Do Predictions
    Linear_predictions = lin_reg.predict(weather_train)
    tree_predictions = tree_reg.predict(weather_train)
    Linear_test_predictions = lin_reg.predict(weather_test)
    tree_test_predictions = tree_reg.predict(weather_test)

    # Print out errors
    tree_mae = mean_squared_error(weather_labels, tree_predictions)
    ...
    tree_test_mae = mean_absolute_error(test_labels, tree_test_predictions)
    ...
    print("Tree MeanAbsoluteError on training data  : " + str(1 - tree_mae) + "%")
    print("Tree MeanAbsoluteError on test data      : " + str(1 - tree_test_mae) + "%")
